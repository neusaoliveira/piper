...
UARTStdioInit(unsigned long ulPortNum)
{
    ...
    UARTStdioConfig(ulPortNum, 115200, MAP_SysCtlClockGet());
      //Para comunicação serial isolada Utilizar 9600 e RF
      //UARTStdioConfig(ulPortNum, 9600, MAP_SysCtlClockGet());
}
...
UARTwrite(const char *pcBuf, unsigned long ulLen)
{
    ...
    for(uIdx = 0; uIdx < ulLen; uIdx++)
    {
        ...
        // Send the character to the UART output.
        //
        MAP_UARTCharPut(g_ulBase, *pcBuf);
...
UARTFlushRx(void)
{
 ...
 /*   unsigned long i;
    for(i=0;i < UART_TX_BUFFER_SIZE;i++)
          {
           g_pcUARTRxBuffer[UART_RX_BUFFER_SIZE] = 0;
          }
*/
    //
...
/*        unsigned long i;
        for(i=0;i < UART_TX_BUFFER_SIZE;i++)
          {
           g_pcUARTTxBuffer[UART_TX_BUFFER_SIZE] = 0;
          }
*/
        //
        // If interrupts were enabled when we turned them off, turn them
