...
//
//*****************************************************************************
#include "lwip/ports/stellaris/perf.c"
#include "lwip/ports/stellaris/sys_arch.c"
#include "lwip/ports/stellaris/netif/stellarisif.c"
...
lwIPPrivateInit(void *pvArg)
{
    ...
    if(g_ulIPMode == IPADDR_USE_STATIC)
    {
        ...
        ip_addr.addr = (g_ulIPAddr);
        //net_mask.addr = htonl(g_ulNetMask);
        net_mask.addr = (g_ulNetMask);
        //gw_addr.addr = htonl(g_ulGWAddr);
        gw_addr.addr = (g_ulGWAddr);
    }
    else
    {
        ip_addr.addr = 0;
